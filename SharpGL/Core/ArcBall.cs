...
namespace SharpGL
{
    ...
    public class ArcBall
    {
        ...
        public void MouseDown(int x, int y, bool camera_z)
        {
            //  Map the start vertex.
            MapToSphere((float)x, (float)y, out startVector, camera_z);
        }

        public void MouseMove(int x, int y, bool camera_z)
        {
            //  todo need solid tuple types.
            //  Calculate the quaternion.
            float[] quaternion = CalculateQuaternion(x, y, camera_z);
        ...
        private void MapToSphere(float x, float y, out Vertex newVector, bool camera_z)
        {
            //float scaledX = x * adjustWidth - 1.0f;
            float scaledX = x * adjustWidth;
            scaledX = scaledX * (float)1.4;
            //float scaledY = 1.0f - y * adjustHeight; 
            float scaledY = y * adjustHeight;
            scaledY = scaledY * (float)1.1;

            float scaledZ = scaledY *(float)Math.Sin(Math.PI / 4);

            //  Get square of length of vector to point from centre.
            float length = scaledX * scaledX + scaledZ * scaledZ;
            length = length * (float)0.5;
            ...
            if (length > 1.0f)
            {
                ...
                newVector = new Vertex(-scaledX, 0, scaledZ);
            }
            else
            {
                //Return a vector to a point mapped inside the sphere.
                newVector = new Vertex(-scaledX, (float)Math.Sqrt(1.0f - length), scaledZ);
            }
        }
